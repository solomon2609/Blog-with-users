...
def login():
    ...
    if form.validate_on_submit():
        if not user:
            ...
            flash("The password is incorrect, please enter the correct password")
            return redirect(url_for("login"))
        else:
            login_user(user)
            return redirect(url_for("get_all_posts"))
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        ...
        db.session.commit()
    return render_template("post.html", post=requested_post, comments=comment_form, current_user=current_user)
